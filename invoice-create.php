...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_invoice') {
    ...
    if (empty($category_ids) || count(array_filter($category_ids)) == 0) {
        ...
        try {
            ...
            $insert_invoice->bind_param("sisdddsddddddddds", 
                $invoice_number, $customer_id, $customer_name, $subtotal, 
                $overall_discount, $overall_discount_type, $grand_total, 
                $total_taxable, 
                ($total_cgst > 0 && $total_taxable > 0) ? ($total_cgst/$total_taxable*100) : 0, 
                $total_cgst,
                ($total_sgst > 0 && $total_taxable > 0) ? ($total_sgst/$total_taxable*100) : 0, 
                $total_sgst,
                $cash_received, $change_give, $pending_amount, $payment_method
            );
            ...
            foreach ($invoice_items as $item) {
                ...
                $insert_item->bind_param("iisisdidddsdddddd", 
                    $invoice_id, $item['product_id'], $item['product_name'], 
                    $item['cat_id'], $item['cat_name'], $item['quantity'], $item['unit'],
                    $item['purchase_price'], $item['selling_price'], $item['discount'], 
                    $item['discount_type'], $item['total'], $item['hsn'],
                    $item['taxable'], $item['cgst'], $item['cgst_amount'], 
                    $item['sgst'], $item['sgst_amount']
                );
