...
// Weekly Loan Statistics
$weekly_loans_query = "SELECT 
...
$weekly_loans_result = mysqli_query($conn, $weekly_loans_query);
$weekly_loans = mysqli_fetch_assoc($weekly_loans_result);

// Weekly Closed Loans
$weekly_closed_query = "SELECT 
...
$weekly_closed_result = mysqli_query($conn, $weekly_closed_query);
$weekly_closed = mysqli_fetch_assoc($weekly_closed_result);

// Weekly Collections
$weekly_collections_query = "SELECT 
...
$weekly_collections_result = mysqli_query($conn, $weekly_collections_query);
$weekly_collections = mysqli_fetch_assoc($weekly_collections_result);

// Weekly Customer Statistics
$weekly_customers_query = "SELECT 
                           COUNT(*) as total_new,
...
$weekly_customers = mysqli_fetch_assoc($weekly_customers_result);

// Weekly Expenses
$weekly_expenses_query = "SELECT 
...
$weekly_expenses = mysqli_fetch_assoc($weekly_expenses_result);

// Weekly Investments
$weekly_investments_query = "SELECT 
...
$weekly_investments_result = mysqli_query($conn, $weekly_investments_query);
$weekly_investments = mysqli_fetch_assoc($weekly_investments_result);

// Weekly Investment Returns
$weekly_returns_query = "SELECT 
...
$weekly_returns_result = mysqli_query($conn, $weekly_returns_query);
$weekly_returns = mysqli_fetch_assoc($weekly_returns_result);

// Weekly Bank Loans
$weekly_bank_query = "SELECT 
                      COUNT(*) as loan_count,
                      SUM(loan_amount) as total_amount,
                      AVG(interest_rate) as avg_rate
                      FROM bank_loans 
                      WHERE DATE(loan_date) BETWEEN '$start_date' AND '$end_date' $branch_where";
$weekly_bank_result = mysqli_query($conn, $weekly_bank_query);
$weekly_bank = mysqli_fetch_assoc($weekly_bank_result);

// Weekly Bank EMI Collections
$weekly_bank_emi_query = "SELECT 
                          COUNT(*) as payment_count,
                          SUM(payment_amount) as total_collected
                          FROM bank_loan_payments 
                          WHERE DATE(payment_date) BETWEEN '$start_date' AND '$end_date' $branch_where";
$weekly_bank_emi_result = mysqli_query($conn, $weekly_bank_emi_query);
$weekly_bank_emi = mysqli_fetch_assoc($weekly_bank_emi_result);

// ==================== DAILY BREAKDOWN FOR THE WEEK (FIXED) ====================
$daily_breakdown_query = "SELECT 
                          ...
                          GROUP BY DATE(l.created_at), DAYNAME(l.created_at)
...
switch ($report_type) {
    ...
    case 'customers':
        // Detailed weekly new customers
        $details_query = "SELECT *, DAYNAME(created_at) as day_name 
                          FROM customers 
                          WHERE DATE(created_at) BETWEEN '$start_date' AND '$end_date' $branch_where
...
// Handle export (keeping existing export code)
if ($export == 'excel' || $export == 'csv') {
    $filename = 'weekly_report_w' . $week . '_' . $year . '.' . ($export == 'excel' ? 'xls' : 'csv');
    ...
    if ($report_type == 'loans') {
        ...
        $headers = ['Weekly Report - Week ' . $week . ', ' . $year];
    }
...
<body>
            ...
            <div class="page-content">
                <div class="reports-container">
                    ...
                    <div class="week-card">
                        ...
                        <form method="GET" action="" id="reportForm">
                            <div class="week-grid">
                                ...
                                <div class="week-actions">
                                    <button type="button" class="btn btn-secondary" onclick="setCurrentWeek()">
                        ...
                        <div class="week-info" style="margin-top: 15px;">
                    ...
                    <div class="report-tabs">
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=summary" 
                           class="tab-btn <?php echo $report_type == 'summary' ? 'active' : ''; ?>">
                            <i class="bi bi-grid"></i> Summary
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=loans" 
                           class="tab-btn <?php echo $report_type == 'loans' ? 'active' : ''; ?>">
                            <i class="bi bi-plus-circle"></i> New Loans
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=collections" 
                           class="tab-btn <?php echo $report_type == 'collections' ? 'active' : ''; ?>">
                            <i class="bi bi-cash-coin"></i> Collections
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=closed" 
                           class="tab-btn <?php echo $report_type == 'closed' ? 'active' : ''; ?>">
                            <i class="bi bi-check2-circle"></i> Closed Loans
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=customers" 
                           class="tab-btn <?php echo $report_type == 'customers' ? 'active' : ''; ?>">
                            <i class="bi bi-people"></i> New Customers
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=expenses" 
                           class="tab-btn <?php echo $report_type == 'expenses' ? 'active' : ''; ?>">
                            <i class="bi bi-wallet2"></i> Expenses
                        </a>
                        <a href="?year=<?php echo $year; ?>&week=<?php echo $week; ?>&branch_id=<?php echo $branch_id; ?>&type=performance" 
                        ...
                        <div class="stats-grid">
                            <div class="stat-card">
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="stat-value"><?php echo intval($weekly_loans['total_loans'] ?? 0); ?></div>
                                    <div class="stat-sub">Amount: ₹<?php echo safeNumberFormat($weekly_loans['total_amount'] ?? 0); ?></div>
                                </div>
                            </div>
                            ...
                            <div class="stat-card">
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="stat-value"><?php echo intval($weekly_closed['total_closed'] ?? 0); ?></div>
                                    <div class="stat-sub">Received: ₹<?php echo safeNumberFormat($weekly_closed['total_received'] ?? 0); ?></div>
                                </div>
                            </div>
                            ...
                            <div class="stat-card">
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="stat-value">₹<?php echo safeNumberFormat(($weekly_collections['total_interest'] ?? 0) + ($weekly_collections['total_principal'] ?? 0)); ?></div>
                                    <div class="stat-sub">Payments: <?php echo intval($weekly_collections['payment_count'] ?? 0); ?></div>
                                </div>
                            </div>
                            ...
                            <div class="stat-card">
                                ...
                                <div class="stat-content">
                                    ...
                                    <div class="stat-value"><?php echo intval($weekly_customers['total_new'] ?? 0); ?></div>
                                    <div class="stat-sub">Referred: <?php echo intval($weekly_customers['referred_count'] ?? 0); ?></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="breakdown-card">
                            ...
                            <div class="table-responsive">
                                <table class="breakdown-table">
                                    ...
                                    <tbody>
                                        <?php
                                        if ($daily_breakdown_result && mysqli_num_rows($daily_breakdown_result) > 0) {
                                            mysqli_data_seek($daily_breakdown_result, 0);
                                            while($day_data = mysqli_fetch_assoc($daily_breakdown_result)) {
                                                $date = $day_data['date'];
                                                $day_name = $day_data['day_name'];
                                                $is_weekend = ($day_name == 'Saturday' || $day_name == 'Sunday');
                                        ?>
                                            <tr class="<?php echo $is_weekend ? 'weekend' : ''; ?>">
                                                <td class="day-highlight"><?php echo substr($day_name, 0, 3); ?></td>
                                                <td><?php echo date('d M', strtotime($date)); ?></td>
                                                <td><?php echo intval($day_data['new_loans']); ?></td>
                                                <td class="amount">₹<?php echo safeNumberFormat($day_data['loan_amount']); ?></td>
                                                <td><?php echo intval($day_data['payments']); ?></td>
                                                <td class="positive">₹<?php echo safeNumberFormat($day_data['interest_collected']); ?></td>
                                                <td class="amount">₹<?php echo safeNumberFormat($day_data['principal_collected']); ?></td>
                                                <td><?php echo intval($day_data['new_customers']); ?></td>
                                                <td class="negative">₹<?php echo safeNumberFormat($day_data['expense_amount']); ?></td>
                                            </tr>
                                        <?php 
                                            }
                                        } else {
                                            // Fallback to generating week days
                                        ...
                                        ?>
                                            <tr class="<?php echo $is_weekend ? 'weekend' : ''; ?>">
                                                <td class="day-highlight"><?php echo substr($day_name, 0, 3); ?></td>
                                                <td><?php echo $current->format('d M'); ?></td>
                                                <td>0</td>
                        ...
                        <div class="summary-section">
                            ...
                            <div class="summary-card">
                                ...
                                <div class="summary-row">
                                    ...
                                    <span class="summary-value"><?php echo intval($weekly_loans['total_loans'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Total Amount:</span>
                                    <span class="summary-value amount">₹<?php echo safeNumberFormat($weekly_loans['total_amount'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Average Amount:</span>
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_loans['avg_loan_amount'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Total Weight:</span>
                                    <span class="summary-value"><?php echo safeNumberFormat($weekly_loans['total_weight'] ?? 0, 3); ?> g</span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Unique Customers:</span>
                                    <span class="summary-value"><?php echo intval($weekly_loans['unique_customers'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Closed Loans:</span>
                                    <span class="summary-value"><?php echo intval($weekly_closed['total_closed'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Avg Duration:</span>
                                    <span class="summary-value"><?php echo safeNumberFormat($weekly_closed['avg_duration'] ?? 0, 0); ?> days</span>
                                </div>
                            </div>
                            ...
                            <div class="summary-card">
                                ...
                                <div class="summary-row">
                                    ...
                                    <span class="summary-value"><?php echo intval($weekly_collections['payment_count'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Interest Collected:</span>
                                    <span class="summary-value positive">₹<?php echo safeNumberFormat($weekly_collections['total_interest'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Principal Collected:</span>
                                    <span class="summary-value amount">₹<?php echo safeNumberFormat($weekly_collections['total_principal'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Overdue Charges:</span>
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_collections['total_charges'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Loans Serviced:</span>
                                    <span class="summary-value"><?php echo intval($weekly_collections['loans_serviced'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row" style="border-top: 2px solid #e2e8f0; margin-top: 10px; padding-top: 10px;">
                                    ...
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_collections['cash_collected'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Bank Transfer:</span>
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_collections['bank_collected'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">UPI:</span>
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_collections['upi_collected'] ?? 0); ?></span>
                                </div>
                            </div>
                            ...
                            <div class="summary-card">
                                ...
                                <div class="summary-row">
                                    ...
                                    <span class="summary-value"><?php echo intval($weekly_customers['total_new'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Referred:</span>
                                    <span class="summary-value"><?php echo intval($weekly_customers['referred_count'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Noted Persons:</span>
                                    <span class="summary-value"><?php echo intval($weekly_customers['noted_persons'] ?? 0); ?></span>
                                </div>

                                <h4 style="font-size: 14px; margin: 15px 0 10px; color: #4a5568;">Expenses</h4>
                                <div class="summary-row">
                                    <span class="summary-label">Total Expenses:</span>
                                    <span class="summary-value negative">₹<?php echo safeNumberFormat($weekly_expenses['total_expenses'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Expense Count:</span>
                                    <span class="summary-value"><?php echo intval($weekly_expenses['expense_count'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Categories:</span>
                                    <span class="summary-value"><?php echo intval($weekly_expenses['expense_types'] ?? 0); ?></span>
                                </div>

                                <h4 style="font-size: 14px; margin: 15px 0 10px; color: #4a5568;">Investments</h4>
                                <div class="summary-row">
                                    <span class="summary-label">New Investments:</span>
                                    <span class="summary-value amount">₹<?php echo safeNumberFormat($weekly_investments['total_investment'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Investment Count:</span>
                                    <span class="summary-value"><?php echo intval($weekly_investments['investment_count'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Returns Processed:</span>
                                    <span class="summary-value">₹<?php echo safeNumberFormat($weekly_returns['principal_returned'] ?? 0); ?></span>
                                </div>

                                <h4 style="font-size: 14px; margin: 15px 0 10px; color: #4a5568;">Bank</h4>
                                <div class="summary-row">
                                    <span class="summary-label">New Bank Loans:</span>
                                    <span class="summary-value amount">₹<?php echo safeNumberFormat($weekly_bank['total_amount'] ?? 0); ?></span>
                                </div>
                                <div class="summary-row">
                                    <span class="summary-label">Bank EMI:</span>
                                    <span class="summary-value positive">₹<?php echo safeNumberFormat($weekly_bank_emi['total_collected'] ?? 0); ?></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="table-card">
                            <div class="table-header">
                                <span class="table-title">
                                    ...
                                    <?php 
                                    switch($report_type) {
                                        case 'loans': echo 'New Loans - Week ' . $week . ', ' . $year; break;
                                        case 'collections': echo 'Collections - Week ' . $week . ', ' . $year; break;
                                        case 'closed': echo 'Closed Loans - Week ' . $week . ', ' . $year; break;
                                        case 'customers': echo 'New Customers - Week ' . $week . ', ' . $year; break;
                                        case 'expenses': echo 'Expenses - Week ' . $week . ', ' . $year; break;
                                        case 'performance': echo 'Employee Performance - Week ' . $week . ', ' . $year; break;
                                    }
                                    ?>
                                        ...
                                        <?php else: ?>
                                            <tr>
                                                <td colspan="10" class="text-center" style="padding: 40px;">
                                                    No records found for Week <?php echo $week; ?>, <?php echo $year; ?>
                                                </td>
                                            </tr>
                                        <?php endif; ?>
                                    </tbody>
    ...
    <script>
        ...
        function setCurrentWeek() {
            ...
            document.querySelector('select[name="week"]').value = week;
        ...
        // Prepare data from actual daily breakdown if available
        <?php
        $chart_labels = [];
        $chart_data = [];
        ...
        if ($daily_breakdown_result && mysqli_num_rows($daily_breakdown_result) > 0) {
            ...
            while($day = mysqli_fetch_assoc($daily_breakdown_result)) {
                $chart_labels[] = substr($day['day_name'], 0, 3);
                $chart_data[] = ($day['interest_collected'] ?? 0) + ($day['principal_collected'] ?? 0);
            }
        } else {
        ...
        new Chart(methodCtx, {
            ...
            data: {
                ...
                datasets: [{
                    data: [
                        <?php echo $weekly_collections['cash_collected'] ?? 0; ?>,
                        <?php echo $weekly_collections['bank_collected'] ?? 0; ?>,
                        <?php echo $weekly_collections['upi_collected'] ?? 0; ?>
                    ],
                    backgroundColor: [
                        '#48bb78',
                        '#4299e1',
