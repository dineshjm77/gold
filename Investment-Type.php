<?php
session_start();
$currentPage = 'investment-type';
$pageTitle = 'Investment Type Management';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'add':
                $investment_type_name = $_POST['investment_type_name'] ?? '';
                $status = isset($_POST['status']) ? 1 : 0;
                
                // Create investment_types table if it doesn't exist
                $insert_query = "INSERT INTO investment_types (investment_type_name, status) 
                                VALUES (?, ?)";
                $stmt = $conn->prepare($insert_query);
                $stmt->bind_param("si", $investment_type_name, $status);
                ...
                if ($stmt->execute()) {
                    ...
                    $log_query = "INSERT INTO activity_log (user_id, action, description, table_name, record_id) 
                                 VALUES (?, 'create', 'Added new investment type', 'investment_types', ?)";
                    $log_stmt = $conn->prepare($log_query);
                    $log_stmt->bind_param("ii", $_SESSION['user_id'], $new_id);
                    $log_stmt->execute();
                    
                    header('Location: Investment-Type.php?success=added');
                    exit();
                } else {
                    $error = "Error adding investment type: " . $stmt->error;
                }
                break;
            ...
            case 'edit':
                ...
                $update_query = "UPDATE investment_types 
                                SET investment_type_name = ?, status = ?
                                WHERE id = ?";
                $stmt = $conn->prepare($update_query);
                $stmt->bind_param("sii", $investment_type_name, $status, $id);
                
                if ($stmt->execute()) {
                    // Log the activity
                    $log_query = "INSERT INTO activity_log (user_id, action, description, table_name, record_id) 
                                 VALUES (?, 'update', 'Updated investment type', 'investment_types', ?)";
                    $log_stmt = $conn->prepare($log_query);
                    $log_stmt->bind_param("ii", $_SESSION['user_id'], $id);
                    $log_stmt->execute();
                    
                    header('Location: Investment-Type.php?success=updated');
                    exit();
                } else {
                    $error = "Error updating investment type: " . $stmt->error;
                }
                break;
            ...
            case 'delete':
                ...
                $delete_query = "DELETE FROM investment_types WHERE id = ?";
                ...
                if ($stmt->execute()) {
                    // Log the activity
                    $log_query = "INSERT INTO activity_log (user_id, action, description, table_name, record_id) 
                                 VALUES (?, 'delete', 'Deleted investment type', 'investment_types', ?)";
                    $log_stmt = $conn->prepare($log_query);
                    $log_stmt->bind_param("ii", $_SESSION['user_id'], $id);
                    $log_stmt->execute();
                    
                    header('Location: Investment-Type.php?success=deleted');
                    exit();
                } else {
                    $error = "Error deleting investment type: " . $stmt->error;
                }
                break;
            ...
            case 'toggle_status':
                ...
                $update_query = "UPDATE investment_types SET status = ? WHERE id = ?";
                $stmt = $conn->prepare($update_query);
                $stmt->bind_param("ii", $new_status, $id);
                
                if ($stmt->execute()) {
                    header('Location: Investment-Type.php?success=status_updated');
                    exit();
                } else {
...
if (isset($_GET['success'])) {
    switch ($_GET['success']) {
        ...
        case 'deleted':
            ...
            break;
        case 'status_updated':
            $message = "Investment type status updated successfully!";
            break;
    }
}
...
$table_check = $conn->query("SHOW TABLES LIKE 'investment_types'");
if ($table_check->num_rows == 0) {
    // Create investment_types table
    $create_table = "CREATE TABLE IF NOT EXISTS `investment_types` (
    ...
    foreach ($sample_data as $data) {
        $insert = $conn->prepare("INSERT INTO investment_types (investment_type_name, status) VALUES (?, ?)");
        $insert->bind_param("si", $data[0], $data[1]);
        $insert->execute();
    }
}

$investment_query = "SELECT * FROM investment_types ORDER BY id ASC";
$investment_result = $conn->query($investment_query);
?>
            ...
            <div class="investment-container">
                ...
                <div class="page-header">
                    <h1 class="page-title">
                        ...
                        Investment Type
                        <span class="badge-count"><?php echo $investment_result ? $investment_result->num_rows : 0; ?></span>
                    </h1>
                    ...
                    <form method="POST" id="mainForm">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" name="investment_type_name" id="mainInvestmentName" class="form-control" 
                                       placeholder="Enter investment type name (e.g., கவராமிகள் கணக்கு)" required>
                            </div>
                        </div>
                ...
                <div class="search-container">
                    <div class="search-box">
                        <input type="text" class="search-input" id="searchInput" placeholder="Search investment types..." onkeyup="searchInvestments()">
                ...
                <div class="table-card">
                    <table class="investment-table" id="investmentTable">
                        ...
                        <tbody>
                            <?php if ($investment_result && $investment_result->num_rows > 0): ?>
                                <?php $sno = 1; ?>
                                <?php while ($row = $investment_result->fetch_assoc()): ?>
                                    <tr>
                                        <td><strong><?php echo $sno++; ?></strong></td>
                                        <td class="investment-name tamil-text">
                                            <i class="bi bi-tag" style="margin-right: 8px; color: #667eea;"></i>
                                            <?php echo htmlspecialchars($row['investment_type_name']); ?>
                                        </td>
                                        <td>
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="4" class="empty-state">
                                        ...
                                        <small>Click "Add Investment Type" to create one.</small>
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
        ...
        <form method="POST" id="investmentForm">
            ...
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <div class="input-group">
                        <i class="bi bi-type input-icon"></i>
                        <input type="text" name="investment_type_name" id="modalInvestmentName" class="form-control" 
                               placeholder="Enter investment type name" required>
                    </div>
                </div>
            ...
            <div class="modal-body">
                <div style="text-align: center; padding: 20px;">
                    ...
                    <p style="color: #4a5568; font-size: 16px; line-height: 1.6;">
                        Are you sure you want to delete this investment type?<br>
...
<script>
    function openAddModal() {
        ...
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-plus-circle" style="margin-right: 8px;"></i> Add Investment Type';
        document.getElementById('investmentId').value = '';
        document.getElementById('modalInvestmentName').value = '';
    ...
    function openEditModal(data) {
        ...
        document.getElementById('modalTitle').innerHTML = '<i class="bi bi-pencil" style="margin-right: 8px;"></i> Edit Investment Type';
        document.getElementById('investmentId').value = data.id;
        document.getElementById('modalInvestmentName').value = data.investment_type_name;
    ...
    function clearForm() {
        document.getElementById('mainInvestmentName').value = '';
    }
    
    function toggleStatus(id, currentStatus) {
        if (confirm('Are you sure you want to ' + (currentStatus ? 'deactivate' : 'activate') + ' this investment type?')) {
    ...
    function searchInvestments() {
        ...
        tableRows.forEach(row => {
            ...
            let investmentName = row.cells[1].textContent.toLowerCase();
            if (investmentName.includes(input)) {
                row.style.display = '';
            } else {
