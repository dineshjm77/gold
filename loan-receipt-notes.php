<?php
session_start();
$currentPage = 'loan-receipt-notes';
$pageTitle = 'Loan Receipt Notes';
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && !empty($search_term)) {
    ...
    $query = "SELECT l.*, 
              ...
              WHERE l.status = 'open'"; // Only open loans
    
    // Add status filter (but always open)
    ...
    if (!empty($from_date) && !empty($to_date)) {
        $query .= " AND DATE(l.receipt_date) BETWEEN ? AND ?";
    } elseif (!empty($from_date)) {
        $query .= " AND DATE(l.receipt_date) >= ?";
...
// Get all open loans for display (if no search)
...
// Get summary statistics - only for open loans
$stats_query = "SELECT 
                ...
                SUM(CASE WHEN status = 'open' THEN loan_amount ELSE 0 END) as open_amount,
                COUNT(DISTINCT customer_id) as unique_customers
                FROM loans
                WHERE status = 'open'";  // Only open loans for stats
...
function getStatusBadge($status) {
    if ($status == 'open') {
        ...
        return '<span class="badge badge-default">' . ucfirst($status) . '</span>';
    }
}
?>
...
<body>
            ...
            <div class="page-content">
                <div class="container">
                    ...
                    <div class="page-header">
                        ...
                        <div>
                            <a href="New-Loan.php" class="btn btn-success">
                                <i class="bi bi-plus-circle"></i> New Loan
                            </a>
                            <a href="closed-loan-receipts.php" class="btn btn-info">
                                <i class="bi bi-archive"></i> View Closed Loans
                            </a>
                        </div>
                    </div>

                    <!-- Summary Statistics - Only Open Loans -->
                    <div class="stats-grid">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-sub">With open loans</div>
                            </div>
                        </div>
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-value"><?php echo formatCurrency($stats['open_amount'] ?? 0); ?></div>
                                <div class="stat-sub">In open loans</div>
                            </div>
                        </div>
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-content">
                                ...
                                <div class="stat-value"><?php 
                                    // Calculate average days for open loans
                                    $avg_query = "SELECT AVG(DATEDIFF(NOW(), receipt_date)) as avg_days FROM loans WHERE status = 'open'";
                                    $avg_result = mysqli_query($conn, $avg_query);
                                    $avg_data = mysqli_fetch_assoc($avg_result);
                                    echo number_format($avg_data['avg_days'] ?? 0);
                                ?></div>
                                <div class="stat-sub">Average loan age</div>
                            </div>
                        </div>
                    </div>
                        ...
                        <form method="GET" action="" id="searchForm">
                            ...
                            <div class="search-actions">
                                ...
                                <button type="submit" class="btn btn-primary">
                                    <i class="bi bi-search"></i> Search Open Loans
                                </button>
                            </div>
                        </form>
                    </div>

                    <!-- Results Table - Only Open Loans -->
                    <div class="table-card">
                        <div class="table-header">
                            <span class="table-title">
                                <i class="bi bi-list-ul"></i>
                                <?php if ($search_performed): ?>
                                    Open Loans matching "<?php echo htmlspecialchars($search_term); ?>"
                                <?php else: ?>
                                    Recent Open Loans
                                <?php endif; ?>
                                <span class="badge badge-info" style="margin-left: 10px;">
                                    ...
                                    if ($search_performed) {
                                        ...
                                        echo $loans_result ? mysqli_num_rows($loans_result) : 0;
                                    }
                                    ?> open loans
                                </span>
                            </span>
                            <div>
                                <a href="loan-receipt-notes.php" class="btn btn-sm btn-secondary">
                        ...
                        <div class="table-responsive desktop-table">
                            <table class="loans-table" id="loansTable">
                                ...
                                <tbody>
                                    <?php 
                                    $result_set = $search_performed ? $search_results : $loans_result;
                                    if ($result_set && mysqli_num_rows($result_set) > 0):
                                        while ($loan = mysqli_fetch_assoc($result_set)):
                                            // Calculate monthly interest
                                            $monthly_interest = ($loan['loan_amount'] * $loan['interest_amount']) / 100;
                                    ?>
                                    <tr>
                                        ...
                                        <td class="text-center"><?php echo $loan['days_old']; ?></td>
                                        <td><?php echo getStatusBadge($loan['status']); ?></td>
                                        <td><?php echo htmlspecialchars($loan['employee_name'] ?? 'N/A'); ?></td>
                                        <td class="text-center">
                                            <div class="action-buttons">
                                                <!-- View Details -->
                                                <a href="view-loan.php?id=<?php echo $loan['id']; ?>" class="action-btn view" title="View Details">
                                                    <i class="bi bi-eye"></i>
                                                </a>
                                                
                                                <!-- Print Receipt -->
                                                <a href="print-receipt.php?id=<?php echo $loan['id']; ?>" class="action-btn print" target="_blank" title="Print Receipt">
                                                    <i class="bi bi-printer"></i>
                                                </a>
                                                
                                                <!-- Download PDF -->
                                                <a href="download-receipt.php?id=<?php echo $loan['id']; ?>" class="action-btn pdf" target="_blank" title="Download PDF">
                                                    <i class="bi bi-file-pdf"></i>
                                                </a>
                                                
                                                <!-- Edit Loan -->
                                                <a href="edit-loan.php?id=<?php echo $loan['id']; ?>" class="action-btn edit" title="Edit Loan">
                                                    <i class="bi bi-pencil"></i>
                                                </a>
                                                
                                                <!-- Close Loan -->
                                                <a href="Close-Loan.php?id=<?php echo $loan['id']; ?>" class="action-btn delete" title="Close Loan" onclick="return confirmClose('<?php echo $loan['receipt_number']; ?>')">
                        ...
                        <div class="mobile-cards">
                            ...
                            <div class="mobile-card open">
                                ...
                                <div class="mobile-card-row">
                                    ...
                                    <span class="mobile-card-value"><?php echo formatDate($loan['receipt_date']); ?></span>
                                </div>
                                <div class="mobile-card-row">
                                ...
                                <div class="mobile-card-row">
                                    ...
                                    <span class="mobile-card-value"><?php echo number_format($loan['net_weight'], 3); ?> g</span>
                                </div>
                                <div class="mobile-card-row">
                                    <span class="mobile-card-label">Interest:</span>
                                    <span class="mobile-card-value"><?php echo $loan['interest_amount']; ?>%</span>
                                </div>
                                <div class="mobile-card-row">
                                    <span class="mobile-card-label">Days:</span>
                                    <span class="mobile-card-value"><?php echo $loan['days_old']; ?></span>
                                </div>
                                <div class="mobile-card-row">
                                    <span class="mobile-card-label">Employee:</span>
                                    <span class="mobile-card-value"><?php echo htmlspecialchars($loan['employee_name'] ?? 'N/A'); ?></span>
                                </div>
                                <div class="mobile-card-actions">
                                    <a href="view-loan.php?id=<?php echo $loan['id']; ?>" class="action-btn view">
                                        <i class="bi bi-eye"></i> View
                                    </a>
                                    <a href="print-receipt.php?id=<?php echo $loan['id']; ?>" class="action-btn print" target="_blank">
                                        <i class="bi bi-printer"></i> Print
                                    </a>
                                    <a href="download-receipt.php?id=<?php echo $loan['id']; ?>" class="action-btn pdf" target="_blank">
                                        <i class="bi bi-file-pdf"></i> PDF
                                    </a>
                                    <a href="edit-loan.php?id=<?php echo $loan['id']; ?>" class="action-btn edit">
                                        <i class="bi bi-pencil"></i> Edit
                                    </a>
                                    <a href="close-loan.php?id=<?php echo $loan['id']; ?>" class="action-btn delete" onclick="return confirmClose('<?php echo $loan['receipt_number']; ?>')">
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#loansTable').DataTable({
                ...
                language: {
                    ...
                    emptyTable: "No open loans found"
                },
                columnDefs: [
        ...
        function resetSearch() {
            window.location.href = 'loan-receipt-notes.php';
        }

        // Confirm close loan
        function confirmClose(receiptNumber) {
            Swal.fire({
                title: 'Close Loan?',
                html: `Are you sure you want to close loan <strong>${receiptNumber}</strong>?`,
                icon: 'warning',
